...
import com.example.crazychat.api.controllers.ws.ParticipantWsController;
    ...
    private void handleLeaveChat(AbstractSubProtocolEvent event) {
        ...
        Optional.ofNullable(headerAccessor.getSessionId())
                ...
                .ifPresent(participant -> {
                    ...
                    log.info(
                            String.format("Participant \"%s\" leave from \"%s\" chat.",
                    ...
                    Optional.ofNullable(setOperations.size(key))
                            .filter(size -> size == 0L)
